!v8: v7 but the weight-if-tree was changed.
module Worm7Vertex_v8
    ...
    subroutine generateWormData(N0,N1,M,n_data) bind(C,name="generate7VertexWormDataFortran_v8") 
	...
	subroutine Worm(N0,N1,M,lattice)
		...
		Wurmloop: do while(check_end==0)
			...
			if (y(1)==0 .and. y(2)==0) then !Falls man aufhört, nimm den Wurmkopf weg
				...
				if (x(1)==start(1) .and. x(2)==start(2)) then !Falls man am Start ist, bleibt der Wurmkopf
					weightx = weight(M,oldvertex_x,1)
				...
				if (y(1)==start(1) .and. y(2)==start(2)) then !Falls man zum Start geht, bleibt der Wurmkopf
