!v7: Same as v6, but x(1),x(2),y(1),y(2) were replaced by arrays x and y.
module Worm7Vertex_v7
    ...
    subroutine generateWormData(N0,N1,M,n_data) bind(C,name="generate7VertexWormDataFortran_v7") 
	...
	subroutine Worm(N0,N1,M,lattice)
		...
		weightx = weight(M,oldvertex_x,0)
		weightx_new = weight(M,oldvertex_x,1)
		q1 = 1.0 / REAL(N0*N1)
        q2 = p0
		...
		Wurmloop: do while(check_end==0)
			...
			if (check_end_prop==0) then
				...
				case(1)
				!Definiere den neuen Punkt (y(1),y(2))
				if (x(1) < N0) then
					y(1) = x(1) + 1
				else !Randbedingung
					y(1) = 1
			...
			if (y(1)==0 .and. y(2)==0) then !Falls man aufhört, nimm den Wurmkopf weg
				weightx = weight(M,oldvertex_x,1)
				weightx_new = weight(M,newvertex_x,0)
			else if (x(1)==start(1) .and. x(2)==start(2)) then !Falls man am Start ist, bleibt der Wurmkopf
				weightx = weight(M,oldvertex_x,1)
			...
			if (y(1)==0 .and. y(2)==0) then !Falls man aufhört, ist y=0.
				weighty = 1.0
				weighty_new = 1.0
			else if (y(1)==start(1) .and. y(2)==start(2)) then !Falls man zum Start geht, bleibt der Wurmkopf
			...
			q1 = proposal(N0,N1,x,y,p0,oldvertex_x)
			q2 = proposal(N0,N1,y,x,p0,newvertex_y)
			...
			if ( pA > u ) then
				...
				if (y(1)==0 .and. y(2)==0) then !Falls man aufhört
					check_end=1
					exit Wurmloop
	...
	function proposal(N0,N1,x,y,p0,oldvertex_x) !Gibt die proposal Wahrscheinlichkeit, von oldvertex_x -> newvertex_x zu gehen.
