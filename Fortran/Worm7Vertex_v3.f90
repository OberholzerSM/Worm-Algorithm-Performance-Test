!v3: Same as v2, but the data is passed between subroutines instead of being shared across the module.
module Worm7Vertex_v3
    ...
    subroutine generateWormData(N0C,N1C,MC,n_data) bind(C,name="generate7VertexWormDataFortran_v3") 
        ...
        integer, dimension(INT(N0C),INT(N1C),4)	 :: lattice     !Lattice of vertices.
        ...
        !Begin Worm
		do i = 2,n_data
			call Worm(INT(N0C),INT(N1C),REAL(MC),lattice)    
        end do
    ...
    subroutine Worm(N0,N1,M,lattice)
        ...
        real :: q1,q2,pA,u,p0                                                       !Proposal and Acceptance probabilities.
		...
		weightx = weight(M,oldvertex_x,.false.)
		weightx_new = weight(M,oldvertex_x,.true.)
		q1 = 1.0 / REAL(N0*N1)
        q2 = p0
        ...
        WormLoop: do while(check_end == .false.)
            ...
            if(check_end_prop == .true.) then
                weightx = weight(M,oldvertex_x,.true.)
                weightx_new = weight(M,newvertex_x,.false.)
                weighty = 1.0
                weighty_new = 1.0
            else 
                ...
                if (x(1) == start(1) .and. x(2) == start(2)) then
                    ...
                    if( sum(newvertex_x) == 3 ) then
                        ...
                        weightx_new = weight(M,newvertex_x,.true.) !.true., since the source remains at the start.
                    end if
                ...
                if (y(1) == start(1) .and. y(2) == start(2)) then
                    weighty = weight(M,oldvertex_y,.true.)        !.true., since there is already a source at the start.
                    weighty_new = weight(M,newvertex_y,.true.)
                !No special conditions: Worm moves to y.
                else
                    weighty = weight(M,oldvertex_y,.false.)
                    weighty_new = weight(M,newvertex_y,.true.)
                end if
            ...
            q1 = proposal(x,y,oldvertex_x,p0,N0,N1)
            q2 = proposal(y,x,newvertex_y,p0,N0,N1)
	...
	function weight(M,vertex,sink) !Weight of a Vertex. sink: Source/Sink at the Vertex.
    ...
    function proposal(x,y,oldvertex_x,p0,N0,N1) !Returns the odds of proposing the step x -> y.
