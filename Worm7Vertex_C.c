...
{
	//Initalize Lattice with calloc.
	enum BondType* lattice = (enum BondType*)calloc(N0*N1*4,sizeof(int));

	//Generate n_data-1 more closed loops.
	for(int i=1; i<n_data;i++)
	{
		Worm_7Vertex_C(N0,N1,M,lattice);
	}
...
void Worm_7Vertex_C(const int N0, const int N1, const double M, enum BondType* const lattice)
{
	...
	const int start[2] = { randomint_C(0,N0-1), randomint_C(0,N1-1) };
	int x[2];
	...
	bool check_end = true;
	{
		...
		const double weightx = weight_7Vertex_C(M,vertex_start,false);
		const double weightx_new = weight_7Vertex_C(M,vertex_start,true);
		const double q1 = 1.0 / (double)(N0*N1);
		const double q2 = p0;
		const double pA = (weightx_new / weightx) * (q2/q1);

		//Test if the start will be accepted.
		const double u = randomdouble_C();
		if(u < pA)
		{
			check_end = false;
			x[0] = start[0];
	...
	while(!check_end)
	{
		...
		if(!check_end_prop)
		{
			...
			const enum Direction direction = (enum Direction)randomint_C(0,3);
		...
		if(check_end_prop)
		{
			weightx = weight_7Vertex_C(M,oldvertex_x,true);
            weightx_new = weight_7Vertex_C(M,newvertex_x,false);
            weighty = 1.0;
            weighty_new = 1.0;
		}
		else
		{
			...
			if( x[0] == start[0] && x[1] == start[1] )
			{
				...
				if( n_bonds >= 3 )
					...
					weightx_new = weight_7Vertex_C(M,newvertex_x,true);
			}
			...
			{
				weighty = weight_7Vertex_C(M,oldvertex_y,true);
				weighty_new = weight_7Vertex_C(M,newvertex_y,true);
			}
			//No special conditions: Worm moves to y.
			else
			{
				weighty = weight_7Vertex_C(M,oldvertex_y,false);
				weighty_new = weight_7Vertex_C(M,newvertex_y,true);
			}
		}

		//Determine the proposal and accpetance probability.
		const double q1 = proposal_7Vertex_C(x,y,oldvertex_x,p0,N0,N1);
		const double q2 = proposal_7Vertex_C(y,x,newvertex_y,p0,N0,N1);
...
double weight_7Vertex_C(double M, const enum BondType vertex[4], bool sink)
{
...
inline int randomint_C(const int a, const int b)
{
	...
	const uint32_t r = pcg32_boundedrand(d);
	return (int)r + a;
}

inline double randomdouble_C()
{
	return (double)pcg32_random() / (double)UINT32_MAX;
}